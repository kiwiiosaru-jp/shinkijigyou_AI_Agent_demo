...
sequenceDiagram
    ...
    DB->>ETL: 保全レコードを取得
    loop 各レコード
        ETL->>LLM: 元テキスト＋既存項目を送信
        LLM-->>ETL: 構造化JSON
(line,equipment,phenomenon,cause,action,
field_corrections,confidence)
        ETL->>EMB: 抽出された用語(設備/現象/原因/処置)を登録
    end
    EMB-->>MAST: クラスタごとのマスタ候補(代表名＋別名リスト)
    LLM->>MAST: クラスタ情報を基に代表名を最終決定
    MAST-->>CLEAN: 確定マスタ＋標準辞書
...
sequenceDiagram
    ...
    participant LLM as GPT-5(構造化&補正案)
    participant EMB as Embedding/クラスタ
    participant MAST as マスタ管理
    participant OUT as クレンジング済DB

    DB->>Batch: レコード一覧(構造化データ)
    loop 各レコード
        Batch->>LLM: レコード(現象,原因,処置,設備,自由記述)
        LLM-->>Batch: JSON構造
{equipment,phenomenon,cause,action,
field_corrections,confidence}
        Batch->>EMB: equipment/phenomenon/cause/action を用語候補として登録
    end
    EMB->>LLM: クラスタごとの用語リストを送信
    LLM-->>MAST: 各クラスタの代表名・分類を提案
    MAST-->>Batch: 確定マスタ・標準辞書を提供
    loop 各レコード
        Batch->>Batch: マスタ・辞書を参照し
項目のズレ補正・表記揺れ統一・欠損補完
        Batch->>OUT: クレンジング済レコード書き込み
    end
```
...
("A-Line","Aライン","Line A",...)
    LineETL->>Norm: ライン文字列の正規化
    Norm-->>Cls: 正規化済みライン名一覧
    Cls->>Cls: Levenshtein距離を計算しクラスタ化
    loop 各クラスタ
        Cls->>LLM: クラスタ内の候補一覧＋簡単な文脈
        LLM-->>Cls: 正式名称, 別名リスト, ライン階層情報
        Cls->>MLine: ラインマスタ登録
(ID, 正式名称, 別名, 階層)
        MLine->>MLine: variant_term → canonical_id の辞書登録
    end
```
...
sequenceDiagram
    ...
    participant LLM as GPT-5(設備/部品抽出)
    participant EMB as Embedding/クラスタ
    participant MEquip as 設備/部品マスタDB

    DB->>ETL: レコード(設備欄＋自由記述)
    loop 各レコード
        ETL->>LLM: テキスト＋既存設備名
        LLM-->>ETL: JSON構造
{line,equipment,equipment_aliases,part}
        ETL->>EMB: equipment/equipment_aliases/part を用語候補として登録
    end
    EMB->>LLM: 用語クラスタ一覧を送信
    LLM-->>MEquip: クラスタ毎の標準設備名/標準部品名/親子関係を決定
...
sequenceDiagram
    ...
    DB->>ETL: 現象/原因/処置テキスト取得
    loop 各レコード
        ETL->>LLM: テキスト送信
        LLM-->>ETL: {phenomenon,cause,action} を抽出
        ETL->>EMB: 用語をEmbedding登録
    end
    EMB->>LLM: クラスタごとの用語リスト送信
    LLM-->>MClass: 大/中/小分類ラベル案を生成
    LLM-->>Dict: canonical_term と variant_term の対応表を生成
    MClass->>MClass: 分類マスタ更新
    Dict->>Dict: 標準用語辞書更新(検索/RAGで利用)
```
...
sequenceDiagram
    ...
    participant LLM as GPT-5(構造化/提案)
    participant Search as Azure AI Search
    participant DV as Dataverse(保全DB/マスタ/辞書)

    User->>App: 現象/対応内容を自由入力
    App->>Flow: 入力テキスト送信(トリガ)
    Flow->>Func: APIリクエスト
    Func->>LLM: 入力テキスト＋マスタ/辞書を渡す
    LLM-->>Func: 構造化JSON
{equipment,phenomenon,cause,action,標準用語候補,不足項目}
    Func->>Search: 構造化情報を用いて類似案件検索
    Search-->>Func: 類似事例一覧
    Func-->>App: 類似事例＋標準用語案＋不足項目の質問
    App-->>User: 「Mtr → モータにしますか？」「部位を選択してください」等を表示
    User->>App: 承認/修正して確定
    App->>Flow: 確定データ送信
...
sequenceDiagram
    ...
    loop 10%サンプル
        LLM-->>EMB: 抽出した用語セットを登録
    end
    EMB->>LLM: クラスタ情報を渡す
    LLM-->>MAST: マスタ骨子案を生成
    Reviewer->>MAST: 骨子案を確認・修正(マスタ確定)
    MAST->>Batch: 確定マスタを提供
    Batch->>DB: 残り90%を読み込み
    loop 各レコード
        Batch->>LLM: レコード＋確定マスタ
        LLM-->>Batch: マッチング結果＋未知候補
        alt 既存マスタにマッチ
...
sequenceDiagram
    ...
    User->>App: 新しい設備/現象を含む報告を入力
    App->>Func: 入力内容を送信
    Func->>LLM: 入力内容＋既存マスタを渡す
    LLM-->>Func: {最も近いマスタ候補, 類似度, 新規追加が妥当かのコメント}
    alt 類似度が高い
        Func-->>App: 既存マスタとの紐づけ案を提示
        User->>App: 承認
        App->>MAST: 既存マスタIDに紐づけ
    else 類似度が低く新規と判断
        Func-->>App: 「新しいマスタとして登録しますか？」と表示
        User->>App: 承認
        App->>MAST: 新しい設備/現象マスタを登録
    end
```
...
sequenceDiagram
    ...
    User->>App: 類似案件を検索
    App->>Search: 検索クエリ送信
    Search-->>App: 検索結果(要確認フラグ付き案件含む)
    User->>App: 要確認案件を選択
    App->>DV: 案件詳細を取得
    alt 要確認フラグあり
        App-->>User: 「原因が未入力です。補足しますか？」と表示
        User->>App: 簡単に補足入力
        App->>Func: 補足内容＋元データ送信
        Func->>LLM: 妥当性・マスタとの整合をチェック
        LLM-->>Func: 修正案/確定案
        Func->>DV: 保全DB更新(原因を埋め/フラグOFF)
    else フラグなし
        App-->>User: 通常の閲覧画面
    end
```
...
```mermaid
graph TD
    User([現場作業員]) -->|入力/検索| App[Power Apps
(保全アプリ)]
