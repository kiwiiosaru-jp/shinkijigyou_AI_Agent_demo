...
| --- | --- | --- | --- | --- | --- |
| 認証・SSO | Entra SAML | ◯ | IdP統合・ドメイン設定・テスト | P1 | 8–12 |
| API/ガバナンス | 全社標準API | △ | API必須化・仕様/SDK・ガードレール | P1 | 5–8 |
| 文書管理ガバナンス | 承認/棚卸/LC | △ | 承認WF・棚卸し・LC導入 | P3 | 10–15 |
...
| RAG KB（既定） | PostgreSQL指定 | △ | Aurora pgvector追加、KB/AOSS既定 | P1/P2 | 25–30 |
| RAG Kendra | Kendra利用 | △ | 任意で維持/不使用 | P1/Opt | 0–10 |
| OCR/表構造保持 | OCR+表保持 | × | Textract前処理→HTML/MD/CSV | P1/P2 | 12–18 |
| PDF大容量/分割 | 50MB超や長文PDFの分割 | × | P1: 分割設定で対応、P2: 自動分割ツールを追加 | P1/P2 | 6–8 |
| チャンク戦略変更 | セマンティック/階層/固定長 | △ | P2: KBチャンク設定を変更（階層/固定長/セマンティック） | P2 | 4–6 |
| データクレンジング/正規化 | ヘッダ/フッタ削除、全角半角/日付揃え | × | P2: 前処理バッチで実装 | P2 | 8–12 |
| 辞書補正/ゆらぎ補正 | ISUZU→いすゞ等 | × | P2: 前処理で辞書補正 | P2 | 6–8 |
| Q&A変換・要約タグ付与 | FAQ化/タグ付与 | × | P3: LLMバッチでQA化/タグ・要約生成 | P3 | 12–18 |
| 管理ダッシュボード/利用状況/ログ | 利用/エラー/エクスポート | × | P1 AWSコンソール (CW/Logs/Athena/CloudTrail)、P2 カスタムUI | P1/P2 | 12–18 |
| ユーザ管理（全体） | 一覧/ロール/停止 | △ | P1 Cognito/Entraコンソール、P2 カスタムUI必要なら | P1/P2 | 5–8 |
| ベクトルDB管理 | 容量/最適化/バックアップ | × | P1 Aurora/KB/AOSSコンソール＋SQL、P2 自動化UI | P1/P2 | 10–15 |
| 参照元表示/リンク | 出典/直リンク/ページ | △ | P1 UIに出典+ページ番号+直リンクを表示 | P1 | 6–10 |
| 検索結果プレビュー | チャンク確認 | △ | P1 抜粋/プレビューを明示表示 | P1 | 4–6 |
| 関連性スコア表示 | スコア表示 | × | P2 UIに数値表示 | P2 | 4–6 |
| フィルター機能 | 日付/メタ絞り込み | △ | P1 既存メタ活用、追加メタはP2拡張 | P1/P2 | 6–10 |
| 類似検索提案 | 履歴ベクトル検索 | × | P2 履歴から類似クエリ提示 | P2 | 8–12 |
| 検索履歴表示 | 履歴一覧/再検索 | × | P2 履歴表示・再実行 | P2 | 6–10 |
| 回答評価 | いいね等 | × | P1 いいね/バッド保存 | P1 | 6–10 |
...
| お知らせ管理 | 障害/告知配信 | × | P2 通知管理UI＋SNS/メール/バナー | P2 | 8–12 |
| 部門ダッシュボード | 部門内利用/人気KW | × | P2 部門ダッシュボード（CW Logs/Athena集計） | P2 | 12–18 |
| 文書/アクセス権限UI | 文書一覧/メタ/ACL | × | P2 カスタムUI＋API（メタ/ACLをDDB等に保持） | P2 | 18–24 |
| 高度検索/結果エクスポート | フィルタ/エクスポート | △ | P2 UIでフィルタ拡張・エクスポート | P2 | 12–18 |
| アップロード/インデックスUI | 一括アップ/進捗/再実行 | × | P2 アップロード＋インデックス状態/再実行UI | P2 | 12–18 |
| マイページ/通知/FAQ | プロフィール/通知/ヘルプ | × | P3 カスタムUI＋通知連携 | P3 | 10–15 |
| Boxパイプライン | Box取込/メタ/通知 | × | P1 最小、P2 差分/Webhook | P1/P2 | 30–40 |
| SharePointパイプライン | SharePoint取込 | × | P2: Graph API＋定期/差分取込→S3、必要に応じ前処理バッチ | P2 | 18–24 |
| Box→pgvector | Box→S3→pgvector | × | API連携実装 | P1 | 12–18 |
| 前処理カスタム | Python差込 | △ | 前処理モジュール化で差替え | P1 | 10–15 |
| リランク（pgvector） | リランク必須 | △ | 再ランキングAPI/内製を追加 | P1/P2 | 10–15 |
| メタ情報・部門別制御 | 部門別アクセス制御 | △ | メタ格納＋APIフィルタ | P1 | 12–18 |
| 共通API改修 | メタフィルタ | △ | APIでメタフィルタ適用 | P1 | 8–12 |
| 管理UI | 容量/部門/お知らせ/ログ | △ | 管理UI拡張 | P2 | 18–24 |
| AgentCore | エージェント基盤 | ◯ | P1で組込み | P1 | 5–8 |
| メタ付与/Q&A | 要約/タグ/Q&A | △ | 前処理で生成系追加 | P2 | 12–18 |
| オートスケール | 要素別スケール | △ | 各コンポーネント設定調整 | P2 | 10–15 |
| 性能 | 応答10–60秒 | ◯ | モデル/チャンク/同時実行/キャッシュ調整 | P1 | 5–8 |
| 可用性 | RPO24h/RTO12h | △ | DDB PITR、Auroraバックアップ/MAZ、Runbook | P1/P2 | 8–12 |
| 監視 | トークン/ファイル/異常検知 | △ | Dashboard/アラート調整 | P1 | 6–8 |
