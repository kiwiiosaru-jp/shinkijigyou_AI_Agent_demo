...
- **マルチテナントは論理分離が最適解**：Entra IDのセキュリティグループ＝TenantID とし、リクエスト前段の「Tenant Resolver Lambda（薄いPythonラッパ）」でテナント決定・ACLフィルタ生成・メタ伝播を行い、既存Node.jsゲートウェイは最小改修。ベクトルDBは `tenant_id`/`allowed_groups` メタでフィルタ。個人ACLは非推奨。
...
## 3. マルチテナント対応方針（今回の議論を反映）
- 分離方針: 物理分離不要、**論理分離（テナントID=Entra Security Group）**で対応。課金も usage plan/ログ集計で論理分離。
- 構成: 既存 LLM Gateway (Node.js) の前段に **Tenant Resolver Lambda (Python薄ラッパ)** を追加し、リクエストから tenantId 抽出＋ACLフィルタ生成＋KB/Kendra/pgvector へのメタ伝播を担当。
- メタデータ: インジェスト時に `tenant_id` / `allowed_groups` をメタとして付与（現状は1:1だが将来拡張可）。検索時に `tenant_id=<tenant>` AND `allowed_groups CONTAINS <group>` でフィルタ。
- アクセス制御: 個人ACLは非推奨（高カーディナリティで管理/性能劣化）。グループ単位 ACL を原則とし、個人レベルは特殊用途のみ出力段で制御。
- 変更影響: 既存ゲートウェイ最小改修で済む（薄ラッパ追加＋メタフィルタ適用）。ベクトルDBの物理分割は不要。

## 4. フィットギャップ一覧（実装フェーズつき、工数は概算）
| 項目 | RFP | 現状 | 方針 | Phase | 人日 |
| --- | --- | --- | --- | --- | --- |
| マルチテナント（論理分離） | 要 | × | Tenant Resolver Lambda（Python）を前段に追加し、Entra SG→tenantId 抽出、`tenant_id/allowed_groups` メタ付与でフィルタ。個人ACLは非推奨、グループACL原則 | P1 | 12–16 |
| システム管理ダッシュボード | Want | × | 総利用者/検索回数/部門数/ストレージ、部門別利用状況、ピーク時間帯、エラー件数種別、ログエクスポートをOSS/OTelベースで提供 | P2 | 12–16 |
| ベクトルDB管理（容量・最適化） | Want | × | 容量確認、インデックス最適化トリガ、バックアップ/復元のRunbook＋簡易UI（OSS/OTel前提） | P2 | 8–12 |
| 部門管理（容量制限/作成編集削除） | Want | × | 部門作成/編集/削除、部門別容量上限設定、残量アラート。DDB/Paramで管理しUI提供（OSS/OTel前提） | P2 | 10–14 |
| お知らせ管理 | Want | × | 管理UIから全/部門向け告知（障害等）。SNS+UIバナー配信（OSS/OTel前提） | P2 | 6–10 |
| 部門ダッシュボード | Want | × | 部門内検索回数・文書数・ストレージ使用率・人気KW表示（OSS/OTel前提） | P2 | 8–12 |
| 文書管理（部門） | Want | × | 文書一覧/メタ編集/タグ/カテゴリ/削除。部門ACLでフィルタ済み | P2 | 12–16 |
| アクセス権設定（文書単位） | Want | × | 部門内公開/限定公開（グループACL）。個人ACLは避け、必要なら出力段で制御 | P2 | 8–12 |
| 検索UI拡張（高度検索/エクスポート） | Want | × | フィルタ（日付/カテゴリ/タグ/部門）、ソート、結果エクスポート | P2 | 8–12 |
| ファイルアップロード/インデックス管理 | Want | × | 複数形式対応、一括アップロード、進捗表示、インデックス状態確認/再実行 | P2 | 10–14 |
| 部門ユーザー管理 | 要 | × | 部門内ユーザー招待/一覧/権限（閲覧/編集）/削除 | P1 | 10–14 |
| 認証・SSO | Entra SAML | ◯ | Cognito（User Pool/Id Pool）は標準でデプロイ済み。EntraとのSAML連携は設定作業（IdPメタデータ登録・属性マッピング・Callback整合）とE2Eテストで対応 | P1 | 6–8 |
...
- フェーズ1: **150–195人日**  
  - 含む: マルチテナント論理分離（Tenant Resolver Lambda、メタ付与/フィルタ組み込み）、部門ユーザー管理（招待/一覧/権限/削除）、検索UI必須機能、RAG配線(KB/AOSS/pgvector)、前処理最小、Box最小取込、ネットワーク/監査/セキュリティ基盤、負荷/セキュリティテスト、AgentCore 等  
  - 除外してP2以降へ: システム管理ダッシュボード、ベクトルDB容量UI、部門ダッシュボード/容量制限/お知らせ、文書管理・インデックス管理・高度検索UIなど（OSS/OTel 前提でP2に移行）
- フェーズ2: **25–40人日**  
