...
**HNSW（Hierarchical Navigable Small World）** は、ベクトル検索（最近傍探索）のための**近似近傍探索（ANN）インデックス**の一種です。  
ベクトルをグラフ構造として保持し、「近そうな点」をたどりながら探索することで、**大規模データでも高速に topK を返す**ことができます。

## どう速くする仕組み？
- データ点（ベクトル）を **近傍グラフ**（各点が近い点へエッジを持つ）として構築
- さらに **階層（Hierarchy）** を作り、
  - 上位層：点が少なく粗い（遠くまでジャンプできる）
  - 下位層：点が多く細かい（最終的な近傍を詰める）
- 探索時は上位層から「良さげな方向へ」降りていくため、全探索に比べ大幅に高速

## 主なパラメータ（チューニング指標）
- **M**：1点あたりの最大近傍（エッジ）数  
  - 大きいほど精度↑・メモリ↑・構築/更新コスト↑
- **efConstruction**：構築時の探索幅  
  - 大きいほど精度↑・構築時間↑
- **efSearch**：検索時の探索幅  
  - 大きいほど精度↑・検索時間↑（一般にオンラインで調整しやすい）

## 特性（設計判断に効くポイント）
- **精度と速度のバランスが良い**（ANNの中でも定番）
- ただし **メモリ使用量が増えやすい**（エッジを持つため）
- **更新（追加）可能**だが、更新頻度が高い/大量だと構築・再構築設計が重要
- 大規模運用では、更新方式（差分・バッチ・ブルーグリーン）やパーティション（tenant単位など）とセットで考えるのが現実的
