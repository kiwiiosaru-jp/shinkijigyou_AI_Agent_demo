...
|------|-----|
| 時間粒度 | **1時間単位** |
| データ期間 | 2023-01-01 ~ 2024-12-31（2年間） |
| 総レコード数 | 約17,520件（24時間 × 365日 × 2年） |
| 訓練期間 | 2023-01-01 ~ 2024-06-30 |
| 検証期間 | 2024-07-01 ~ 2024-09-30 |
| テスト期間 | 2024-10-01 ~ 2024-12-31 |

### 3.2 処理フロー概要

本システムの処理は、**学習フェーズ**と**予測フェーズ**の2段階で構成される。

```mermaid
flowchart TD
    subgraph Phase1["Phase 1: 学習フェーズ（訓練データ使用）"]
        T1[(訓練データ<br/>2023/1〜2024/6)] --> T2[LightGBM<br/>モデル学習]
        T1 --> T3[特異点閾値決定<br/>気温p5/p95<br/>トレンドσ×2]
        T1 --> T4[RAGナレッジ<br/>ベース構築]
    end

    subgraph Phase2["Phase 2: 予測フェーズ（テストデータ1件ずつ処理）"]
        P1[テストデータ<br/>1時間分] --> P2["① ベース予測<br/>LightGBM推論"]
        P1 --> P3["② 特異点判定<br/>閾値と比較"]

        P3 -->|通常日| P4[ベース予測値<br/>をそのまま採用]
        P3 -->|特異点| P5["③ RAG検索<br/>類似事例取得"]
        P5 --> P6["④ LLM呼出<br/>補正率判断"]
        P6 --> P7["⑤ 補正適用<br/>base×(1+adj)"]
...
特異点の閾値は**訓練データのみ**から決定され、テストデータの各レコードに対して1件ずつ判定を行う。

#### 閾値の決定方法（学習フェーズ）

| 閾値 | 算出方法 | 具体例 |
|------|----------|--------|
| 気温上限 | 訓練データの95パーセンタイル | 約32.5℃ |
| 気温下限 | 訓練データの5パーセンタイル | 約5.2℃ |
| トレンド閾値 | 24時間差分の標準偏差 × σ係数(2.0) | 約±850kW |

#### 判定条件（予測フェーズ）

以下の**いずれかに該当**すれば特異点と判定：

```mermaid
flowchart LR
    subgraph Conditions["特異点判定条件（OR）"]
        C1["気温 > 上限閾値<br/>（猛暑日）"]
        C2["気温 < 下限閾値<br/>（厳寒日）"]
        C3["is_holiday = True<br/>（祝日）"]
        C4["|現在-24h前| > トレンド閾値<br/>（急変動）"]
    end
...
補正は**ベース予測値に対して**適用される。LLMは予測値そのものではなく、補正率（adjustment_rate）を出力する。

#### 処理ステップ（1件のテストデータに対して）

```
【入力例】2024-12-25 14:00（クリスマス・祝日）

① ベース予測（LightGBM）
   → 6,500 kW

② 特異点判定
   → is_holiday=True → 「特異点」と判定

③ RAG検索
   → 類似事例3件取得
   → 平均乖離率: +18%（実績がベース予測より18%高かった）
...
⑤ 補正計算
   RAG補正 = β × (avg_mult - 1.0) = 0.35 × 0.18 = 0.063
   LLM補正 = α × adj_rate = 0.3 × 0.12 = 0.036
   合成補正率 = 0.063 + 0.036 = 0.099 (+9.9%)

⑥ 最終予測
   補正後予測 = 6,500 × (1 + 0.099) = 7,144 kW
```
...
| 構成 | 補正式 | 説明 |
|------|--------|------|
| ベースライン | `final = base` | 補正なし |
| RAG+LLM | `final = base × (1 + β×rag_adj + α×llm_adj)` | RAG主体 + LLM微調整 |
| RAGのみ | `final = base × (1 + β×rag_adj)` | RAG補正のみ |
| LLMのみ | `final = base × (1 + α×llm_adj)` | LLM補正のみ |

※ α=0.3（LLM係数）、β=0.35（RAG係数）
