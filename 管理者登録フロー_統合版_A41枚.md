...
全社共通LLMゲートウェイ基盤のナレッジベースは、**Aurora PostgreSQL（pgvector）**を採用し、**高精度なデータ前処理**と**精密なテーブル設計・チューニング**により、コストパフォーマンスに優れた全社展開を実現します。

### Aurora PostgreSQL（pgvector）選定理由

| 選定理由 | 説明 |
|---------|------|
| **コストパフォーマンス** | Aurora Serverless v2 採用により従量課金で月額数千円から全社展開が可能。OpenSearch Serverless（月額$700〜）と比較して**90%以上のコスト削減**。 |
| **AWSマネージドサービス** | Bedrock Knowledge Base が公式サポートするベクトルデータベース。バックアップ、パッチ適用、スケーリングが自動化。 |
| **ハイブリッド検索** | pgvector（ベクトル類似度検索）+ PostgreSQL RDB機能（キーワード検索、ファイル名検索、作成日フィルタリング、部門フィルタ）を単一データベースで実現。 |
| **高速検索** | HNSW（Hierarchical Navigable Small World）インデックスによる高速ベクトル検索。数百万チャンクでもミリ秒オーダーの応答。 |
| **マルチテナント対応** | `department` カラムによる部門別データ管理。複雑な検索条件でのインデックス最適化。 |
| **エンタープライズ対応** | パーティショニング、自動バキューム設定、接続プーリング等のRDBMSチューニングにより数千ファイル・数百万チャンクに対応。 |
...
### Aurora PostgreSQL（pgvector）の技術的特徴

| 技術要素 | 実装内容 | 効果 |
|---------|---------|------|
| **pgvector拡張** | PostgreSQLのオープンソース拡張機能。ベクトル型（`vector`）とベクトル類似度検索演算子（`<=>`, `<->`, `<#>`）を提供。 | Bedrock Embeddings（1536次元）をネイティブに格納・検索可能 |
| **HNSWインデックス** | 階層型グラフ構造により近似最近傍探索を高速化。OpenSearch と同等の検索速度（ミリ秒オーダー）を実現。<br>`CREATE INDEX ON bedrock_kb USING hnsw (embedding vector_cosine_ops);` | 数百万チャンクでも高速検索 |
| **部門別フィルタ** | `department` カラムに部門ID（SG-Sales等）を格納し、B-treeインデックスで高速フィルタリング。<br>`CREATE INDEX ON bedrock_kb (department);` | テナント別データ分離 |
| **ハイブリッド検索** | pgvector（ベクトル検索）+ GiNインデックス（全文検索）+ B-treeインデックス（メタデータ検索）の組み合わせ。<br>`CREATE INDEX ON bedrock_kb USING gin (to_tsvector('simple', chunks));` | 柔軟な検索条件 |
| **パーティショニング** | 部門別テーブルパーティション（`PARTITION BY LIST (department)`）によりクエリ性能向上とメンテナンス性向上。 | 大規模データ対応 |
| **自動バキューム** | `autovacuum_naptime`, `autovacuum_vacuum_scale_factor` 調整により、削除・更新時のテーブル肥大化を防止。 | 長期運用の性能維持 |
| **接続プーリング** | RDS Proxy 経由でコネクション管理。Lambda の大量同時実行でも接続枯渇を回避。 | 高スループット対応 |
