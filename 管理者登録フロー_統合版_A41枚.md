...
```
【一般ユーザー（佐藤さん）がチャットを開始する例】

1. ユーザー操作
   └─ 佐藤さんがブラウザで https://llm-gateway.isuzu.co.jp にアクセス

2. SSO認証
   ├─ Cognito Hosted UI が表示
   ├─ 「Entra ID でログイン」をクリック
   └─ Entra ID 認証成功
       └─ SAML Assertion 返却（所属: SG-Sales）

3. マルチテナントリゾルバ処理
   ├─ Cognito User Pool にユーザー自動作成（sato@isuzu.co.jp）
   ├─ Entra IDから受信したセキュリティグループ（SG-Sales）を確認
   ├─ 対応するCognitoグループを判定: User-SG-Sales
   └─ JWT発行
       └─ cognito:groups: ["User-SG-Sales"]
       └─ custom:tenantId: "SG-Sales" ← テナントIDを明示的に付与

4. チャットアプリ起動
   ├─ React SPAがJWTをLocalStorageに保存
   ├─ JWT解析: cognito:groups = ["User-SG-Sales"]
   ├─ テナントID抽出: "SG-Sales"
   └─ 画面表示: "営業部のRAGチャット" ← 部門名を表示

5. LLMゲートウェイへの連携
   └─ 以降のすべてのAPI呼び出しで、Authorization: Bearer {JWT} を付与
       └─ LLMゲートウェイがJWTからテナントID "SG-Sales" を抽出
```
