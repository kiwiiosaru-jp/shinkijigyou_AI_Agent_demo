...
本提案で「従来のOCR」と表現しているのは、以下のツール・ライブラリを指します:

| カテゴリ | 具体例 | 主な問題点 |
|---------|-------|-----------|
| **オープンソースOCR** | Tesseract OCR | 表構造を認識できず、セル内のテキストが混在。精度90%前後で条件により大幅低下。 |
| **汎用PDFライブラリ** | PyPDF2, PyMuPDF (fitz), pdfplumber | 単純なテキスト抽出のみ。表領域の行列関係を保持できない。レイアウト解析が弱い。 |
| **汎用OCRサービス** | Google Cloud Vision OCR等 | 表構造認識は限定的。Textract TABLESオプションのような専用機能がない。 |

### カスタム前処理パイプラインのコンポーネント構成

本提案の**カスタム前処理パイプライン**では、以下のコンポーネントで高度な処理を実現しています:

| 機能 | 実装コンポーネント | 処理内容 | 開発要素 |
|------|------------------|---------|---------|
| **表構造認識** | **Amazon Textract**（AWSマネージドサービス） | FeatureTypes: ['TABLES', 'LAYOUT'] 指定により、セル構造・行列関係を完全保持したMarkdown形式に変換 | AWS標準機能（API呼び出しのみ） |
| **セマンティック境界認識** | **Lambda: DocumentProcessing**<br>`smartChunking()` 関数（**当社カスタム開発**） | - 段落境界（`\n\n+`）での分割<br>- 見出しパターン認識（`#`, `##`, `第〇章` 等）<br>- 正規表現による意味的なまとまりの保持<br>- 不自然な位置での分割を回避 | 当社カスタム開発<br>（DocumentProcessing Lambda: 8人日） |
| **親子関係保持** | **Aurora PostgreSQL**<br>テーブルスキーマ設計（**当社カスタム開発**） | - `original_doc_id`: 親ドキュメントID（S3キー）<br>- `extracted_id`: 子チャンク固有ID（`{S3キー}-{index}`）<br>- `file_name`: ファイル名<br>- `chunk_index`: チャンク番号<br>→ ファイル単位の親と各チャンクの関係を明示的に格納 | 当社カスタム開発<br>（Aurora CDK: 3人日） |
| **Embeddings生成** | **Bedrock Embeddings API**（AWSマネージドサービス） | amazon.titan-embed-text-v1 を使用し、1536次元ベクトルを生成 | AWS標準機能（API呼び出しのみ） |
| **ベクトル格納** | **Aurora PostgreSQL (pgvector拡張)** | HNSW インデックスによる高速ベクトル検索。部門別パーティション。 | AWS標準機能 + 当社設定 |

**重要**: 本提案では**Bedrock Knowledge Baseのマネージドインジェスト機能は使用せず**、上記のカスタム前処理パイプラインを構築します。これにより、Textractの表構造認識と独自のセマンティック境界認識チャンキングを組み合わせた、高精度なRAG検索を実現します。

### データ取り込み方式の比較

以下、4つのアプローチを比較します:

| 項目 | **① カスタムTextract<br>パイプライン<br>（本提案）** | **② KB マネージド<br>インジェスト<br>（標準）** | **③ KB マネージド<br>+ 高度なパース設定** | **④ KB マネージド<br>+ Claude<br>マルチモーダル** |
|------|------------------------------------------|----------------------------------------|----------------------------------|------------------------------------------|
| **OCR/解析エンジン** | Amazon Textract<br>TABLES + LAYOUT | AWS管理下の解析エンジン<br>（詳細非公開） | AWS管理下の解析エンジン<br>+ Advanced Parsing | Claude Sonnet<br>（Vision API） |
| **表領域の読み取り精度** | **★★★★★**<br>セル構造を完全保持。Markdown形式でLLMが理解可能。財務諸表・仕様書等で威力を発揮。 | **★★☆☆☆**<br>表構造の認識は限定的。セル境界が曖昧になり、検索精度が低下。 | **★★★☆☆**<br>改善されるが、Textract TABLESオプションには及ばない。 | **★★★★☆**<br>Claudeが画像から表を理解。精度は高いが、トークン消費が大きい。 |
| **複雑なレイアウト対応** | **★★★★★**<br>2カラム・3カラムを正しく認識。図表とテキストを分離。 | **★★☆☆☆**<br>複雑なレイアウトで破綻しやすい。 | **★★★☆☆**<br>Advanced Parsingで改善。 | **★★★★☆**<br>Claudeが視覚的に理解するため対応可能。 |
| **セマンティック境界認識<br>チャンキング** | **★★★★★**<br>カスタムLambda `smartChunking()` で見出し・段落単位での分割を実装。文脈を保持。 | **★★★☆☆**<br>固定トークン数での分割。セマンティック境界を考慮しない。 | **★★★☆☆**<br>同上 | **★★★☆☆**<br>同上 |
| **親子関係保持** | **★★★★★**<br>Aurora PostgreSQLスキーマで `original_doc_id` / `extracted_id` を明示的に管理。 | **★★★☆☆**<br>Knowledge Baseが自動管理するが、カスタマイズ不可。 | **★★★☆☆**<br>同上 | **★★★☆☆**<br>同上 |
| **処理コスト<br>（1,000ページ想定）** | **約$15〜25**<br>- Textract: $15（1ページ$0.015 × 1,000）<br>- Lambda実行: $5<br>- Embeddings: $5 | **約$10〜20**<br>KB Data Sourceの自動取り込み料金<br>（詳細は非公開だが、概ね同等） | **約$15〜25**<br>Advanced Parsing追加料金 | **約$80〜150**<br>- Claude Sonnet画像入力:<br>  1ページ$0.08〜0.15 × 1,000<br>（トークン消費が非常に大きい） |
| **開発工数** | **18人日**<br>- Lambda実装: 13人日<br>- インフラ（CDK）: 5人日 | **5人日**<br>- Data Source設定<br>- S3バケット連携<br>- スケジュール設定 | **7人日**<br>- 同上 + Advanced Parsing<br>  設定とテスト | **10人日**<br>- 同上 + Claude multimodal<br>  連携実装とトークン最適化 |
| **カスタマイズ柔軟性** | **★★★★★**<br>チャンキング戦略、メタデータ付与、部門別処理を自由にカスタマイズ可能。 | **★☆☆☆☆**<br>KB Data Sourceの設定範囲内のみ。チャンキング戦略の変更不可。 | **★★☆☆☆**<br>Advanced Parsing設定でやや改善。 | **★★☆☆☆**<br>Claude Vision APIの出力に依存。チャンキング戦略は変更不可。 |
| **メンテナンス性** | **★★★☆☆**<br>Lambda関数の保守が必要。Textract API変更への追従。 | **★★★★★**<br>AWSがフルマネージド。アップデート自動適用。 | **★★★★☆**<br>同上。Advanced Parsing設定の見直しが必要な場合あり。 | **★★★☆☆**<br>Claude API変更への追従が必要。トークン消費の監視が重要。 |
| **適用ケース** | **✅ 推奨**<br>- 財務諸表・仕様書・マニュアル等の**表データが重要**な文書<br>- 複雑なレイアウト（2カラム、図表混在）<br>- セマンティック境界認識が必要<br>- 独自のチャンキング戦略が必要 | 標準的なテキスト中心の文書<br>表データが少ない<br>迅速な導入が優先 | テキスト中心だが<br>やや複雑なレイアウト<br>Advanced Parsingで<br>十分カバー可能 | 画像・図表が多い文書<br>Claudeの視覚理解が必要<br>コスト許容度が高い |

### 本提案の選定理由

**① カスタムTextractパイプライン**を採用する理由:

1. **表領域の高精度読み取りが必須**: いすゞ様の全社RAG基盤では、財務諸表・技術仕様書・マニュアル等、**表データを含むドキュメントの正確な検索が要件**です。Textract TABLESオプションによるセル構造保持が、LLMの正確な回答生成に不可欠です。

2. **セマンティック境界認識によるコンテキスト保持**: 見出し・段落単位でのチャンキングにより、文脈を保持した検索精度向上を実現します。KB標準のインジェストでは固定トークン数での分割となり、不自然な位置で分割される可能性があります。

3. **柔軟なカスタマイズ対応**: 部門別の特別な処理、メタデータ付与、チャンキング戦略の調整等、今後の要件変更に柔軟に対応可能です。

4. **コストパフォーマンス**: Claude multimodal（④）と比較して**トークン消費を80%削減**し、長期運用でのコスト優位性があります。
...
全社共通LLMゲートウェイ基盤では、**最小限の管理UI**と**CloudWatch Dashboard によるRAG関連モニタリング**を提供します。PH1では、部門管理とユーザー招待に必要な最小限の機能のみを実装し、高度なダッシュボードや分析機能は次期フェーズで対応します。

### PH1実装スコープ

| カテゴリ | 機能 | 実装内容 | 開発要素 |
|---------|------|---------|---------|
| **システム管理者<br>向け管理UI** | 部門管理 | - 部門一覧表示<br>- 部門作成・削除<br>- 部門管理者招待 | **当社カスタム開発**<br>React管理画面<br>+ Lambda API |
| | ユーザー管理 | - Cognito User Pool の管理画面で全ユーザー覧表示<br>- ユーザーアカウント停止は Entra ID 側で対応<br>- EntraID で削除されたユーザー情報の同期は次期フェーズで対応 | **Cognito標準機能**<br>+ 次期フェーズ拡張 |
| | ベクトルDB管理 | - 管理UI開発は行わず、AWSコンソールおよび定常運用スクリプト（バックアップ等）にて対応<br>- インデックス最適化は Aurora の自動処理に委ねる | **AWS標準機能**<br>+ 運用スクリプト |
| **部門管理者<br>向け管理UI** | ユーザー管理 | - 部門内ユーザー一覧表示<br>- 一般ユーザー招待（招待登録）<br>- 利用開始と停止を行うSUI | **当社カスタム開発**<br>React管理画面<br>+ Lambda API |
| | ファイル<br>アップロード | - PH1ではシステム管理者によるS3への手動バックグラウンドアップロードのみ<br>- 画面側は次期フェーズで対応 | **次期フェーズ** |
| **RAG関連<br>モニタリング** | CloudWatch<br>Dashboard | - テナント別トークン使用量<br>- 部門ごとのコスト観測<br>- ベクトルDB容量<br>- API レスポンス時間 | **当社カスタム開発**<br>CloudWatch Dashboard<br>+ Metric Filter<br>+ Lambda ログ出力 |

### 次期フェーズ拡張機能（PH1ではスコープ外）

| カテゴリ | 機能 | 説明 |
|---------|------|------|
| **高度なダッシュボード** | 利用状況分析 | - 日次/月次の検索回数推移<br>- 部門別利用状況<br>- ピーク時間帯分析<br>- よく検索されるキーワード |
| | エラー監視 | - エラー発生件数、エラー種別<br>- インデックス失敗<br>- 検索エラー等の詳細分析 |
| | ログ管理 | - ファイルアップロード履歴<br>- 検索履歴<br>- ユーザー操作履歴<br>- ログエクスポート |
| **文書管理UI** | 文書管理 | - 文書一覧<br>- メタデータ編集<br>- タグ付・カテゴリ分類<br>- 文書削除 |
| | アクセス権限設定 | - 文書単位のアクセス制御<br>- 部門内公開/限定公開設定 |
| **ファイル<br>アップロードUI** | アップロード画面 | - ファイル選択<br>- アップロード進捗表示<br>- インデックス作成状態確認 |
| | エラー処理 | - エラードキュメント検出<br>- リトライ機能 |

### 管理画面アーキテクチャ

```
┌──────────────────────────────────────────────────────────────┐
│           System Administration & Monitoring Architecture      │
├──────────────────────────────────────────────────────────────┤
│                                                                │
│  [システム管理者 / 部門管理者]                                 │
│           │                                                    │
...
│  ┌────────────────┐                                           │
│  │ API Gateway    │ ② 認証・認可                              │
│  │ + Cognito      │   - JWT検証（Admin-* グループ）           │
│  │   Authorizer   │   - 管理者権限チェック                    │
│  └────────────────┘                                           │
│           │                                                    │
│           ▼                                                    │
│  ┌────────────────┐                                           │
│  │ Lambda:        │ ③ 管理操作                                │
│  │ - CreateTenant │   - Cognitoグループ作成                   │
│  │ - AssignUser   │   - ユーザーグループ割り当て              │
│  │ - ListUsers    │   - ユーザー一覧取得                      │
│  └────────────────┘                                           │
│           │                                                    │
│           ▼                                                    │
...
### CloudWatch Dashboard（RAG関連モニタリング）

PH1では、RAG関連のモニタリングに特化したCloudWatch Dashboardを構築します。

#### モニタリング項目

| メトリクス | データソース | 実装方法 | 可視化 |
|-----------|------------|---------|--------|
| **テナント別<br>トークン使用量** | Lambda ログ<br>（ChatGateway） | Lambda がAPI呼び出しログに以下を出力:<br>`{ tenantId, userId, model, inputTokens, outputTokens }`<br>→ CloudWatch Logs Insights でクエリ<br>→ Metric Filter で集計 | グラフ:<br>- 部門別トークン使用量（棒グラフ）<br>- 時系列トークン推移（折れ線グラフ） |
| **部門ごとの<br>コスト観測** | Lambda ログ<br>+ 料金テーブル | 上記トークン使用量に料金レートを掛け算:<br>- Claude 3.5 Sonnet: 入力$3/1M、出力$15/1M<br>- Claude 3 Haiku: 入力$0.25/1M、出力$1.25/1M<br>→ 部門別コストを算出 | グラフ:<br>- 部門別コスト（円グラフ）<br>- 月次コスト推移（折れ線グラフ） |
| **ベクトルDB容量** | Aurora<br>PostgreSQL | CloudWatch標準メトリクス:<br>- `DatabaseConnections`<br>- `FreeStorageSpace`<br>- `CPUUtilization`<br>- `FreeableMemory` | グラフ:<br>- ストレージ使用量（ゲージ）<br>- 接続数（折れ線グラフ） |
| **API レスポンス<br>時間** | API Gateway<br>Lambda | CloudWatch標準メトリクス:<br>- `IntegrationLatency`<br>- `Latency`<br>- `Duration`（Lambda） | グラフ:<br>- レスポンス時間分布（ヒートマップ）<br>- P50/P90/P99（折れ線グラフ） |
| **エラー率** | API Gateway<br>Lambda | CloudWatch標準メトリクス:<br>- `4XXError`<br>- `5XXError`<br>- `Errors`（Lambda） | グラフ:<br>- エラー件数（棒グラフ）<br>- エラー率（折れ線グラフ） |

#### CloudWatch Dashboard 実装例

```typescript
// CDK: CloudWatch Dashboard 定義
import { Dashboard, GraphWidget, Metric } from 'aws-cdk-lib/aws-cloudwatch';

const dashboard = new Dashboard(this, 'RAGMonitoringDashboard', {
  dashboardName: 'GenU-RAG-Monitoring',
});

// 1. テナント別トークン使用量（Metric Filter から生成）
const tokenMetric = new Metric({
  namespace: 'GenU/RAG',
  metricName: 'TokenUsage',
  dimensionsMap: {
    TenantId: 'SG-Sales',  // 部門ごとに生成
  },
  statistic: 'Sum',
});
...
// 2. ベクトルDB容量（Aurora標準メトリクス）
const dbStorageMetric = new Metric({
  namespace: 'AWS/RDS',
...
// 3. API レスポンス時間（API Gateway標準メトリクス）
...
#### Metric Filter 設定例（テナント別トークン使用量）

```typescript
// CDK: Metric Filter 定義
import { MetricFilter, FilterPattern } from 'aws-cdk-lib/aws-logs';

new MetricFilter(this, 'TokenUsageMetricFilter', {
  logGroup: chatGatewayLogGroup,
  filterPattern: FilterPattern.exists('$.inputTokens'),
  metricNamespace: 'GenU/RAG',
  metricName: 'TokenUsage',
...
### Lambda ログ出力実装（テナントID埋め込み）

```typescript
// Lambda: ChatGateway（抜粋）
export const handler = async (event: APIGatewayProxyEvent) => {
  const claims = event.requestContext.authorizer!.claims;
  const tenantId = extractTenantId(claims['cognito:groups']);
  const userId = claims['email'];
  const { message, model } = JSON.parse(event.body!);

  // ガバナンスログ記録（テナントID埋め込み）
  console.log(JSON.stringify({
    timestamp: new Date().toISOString(),
    tenantId,           // ← テナントID
    userId,
...
### 開発工数（システム管理・モニタリング基盤）

| カテゴリ | 開発要素 | 使用技術 | 工数見積 |
|---------|---------|---------|---------|
| **管理画面UI** | 部門管理画面（システム管理者） | React, TypeScript, Material-UI | **5人日** |
| | ユーザー管理画面（部門管理者） | React, TypeScript, AWS SDK | **12人日** |
| **Lambda API** | CreateTenant, AssignUser, ListUsers | TypeScript, AWS SDK | **10人日** |
| **CloudWatch<br>Dashboard** | Dashboard定義、Metric Filter、ログ出力 | CDK, TypeScript | **5人日** |
| **インフラ（CDK）** | API Gateway, Cognito設定 | CDK, TypeScript | **含む**（認証・認可システムに含む） |

**システム管理・モニタリング基盤 小計**: **32人日**

※ 認証・認可システム（55人日）と一部重複するため、全体工数には**追加分のみ計上**（約10人日）
...
一般利用者向けのチャットUIは、**GenU標準のチャット/RAG画面をベース**に、部門別フィルタリングと引用表示機能を統合した、シンプルで直感的なインターフェースを提供します。

### PH1実装スコープ

| カテゴリ | 機能 | 実装内容 | 開発要素 |
|---------|------|---------|---------|
| **基本チャット機能** | メッセージ送信・受信 | - テキスト入力フォーム<br>- ストリーミングレスポンス表示（SSE）<br>- マークダウンレンダリング<br>- コードブロックシンタックスハイライト | **GenU標準機能**<br>React SPA<br>+ LLMゲートウェイ統合 |
| **RAG検索・引用表示** | 検索元の表示と<br>元ドキュメント追跡 | - 回答に使用されたチャンク情報の表示<br>- ファイル名・チャンク位置の表示<br>- **S3署名付きURL**による元ドキュメントへのアクセス<br>- チャンクプレビュー（検索結果の一部表示） | **当社カスタム開発**<br>Lambda<br>(GeneratePresignedUrl)<br>+ フロントエンド統合 |
| **検索履歴** | セッション管理 | - サイドバーでの会話履歴表示<br>- 過去の会話の再開<br>- 会話の削除 | **GenU標準機能**<br>DynamoDB セッション管理 |
| **フィードバック機能** | 回答評価（Good/Bad） | - 各回答にGood/Badボタン表示<br>- 評価結果をDynamoDBに蓄積<br>- 精度チューニングの材料として格納 | **GenU標準機能**<br>+ DynamoDB保存 |
| **外部フィードバック** | 詳細コメント | - SharePoint（またはMicrosoft Forms）へのリンク/ボタン設置<br>- GenU内でのコメント入力機能は実装しない | **当社カスタム開発**<br>静的リンク設置のみ |
| **モデル選択** | LLMモデル切り替え | - Claude 3.5 Sonnet / Claude 3 Haiku 等の選択UI<br>- 選択したモデルをAPIリクエストに含める | **GenU標準機能**<br>ドロップダウンUI |
| **テナント別フィルタリング** | 自動適用 | - JWTの `cognito:groups` からテナントID自動抽出<br>- LLMゲートウェイが自動的に部門別フィルタを適用<br>- ユーザーは操作不要（透過的） | **当社カスタム開発**<br>LLMゲートウェイ<br>（既存実装） |

### 次期フェーズ拡張機能（PH1ではスコープ外）

| カテゴリ | 機能 | 説明 |
|---------|------|------|
| **高度な検索** | フィルター機能 | - 日付範囲フィルター<br>- カテゴリフィルター<br>- ファイル形式フィルター<br>- メタデータ検索 |
| | ソート機能 | - 関連度順<br>- 日付順<br>- ファイル名順 |
| **検索分析** | 各参照元の関連度を数値化 | - スコア表示<br>- 信頼度インジケーター |
| | 類似検索の提案 | - 過去の検索履歴をベクトル検索<br>- 関連する質問の提案 |
| **UI拡張** | サイト一での開催表示 | - モデル詳細<br>- メッセージ統計 |
| | ダーク/ライトモード | - テーマ切り替え<br>- ユーザー設定保存 |

### チャットUI画面構成

```
┌──────────────────────────────────────────────────────────────┐
│               GenU RAG Chat - 営業部（SG-Sales）              │
├──────────────────────────────────────────────────────────────┤
│                                                                │
│  [サイドバー: 会話履歴]    │  [メインエリア: チャット画面]   │
│                            │                                  │
│  📁 今日                   │  👤 User:                        │
│    - 売上レポート分析     │  「2024年Q3の売上レポートを...」│
│    - 顧客データ検索       │                                  │
│                            │  🤖 Assistant:                   │
│  📁 昨日                   │  「2024年Q3の売上は...」         │
│    - 製品仕様確認         │                                  │
│                            │  📎 引用元:                      │
│  [+ 新しい会話]            │  - 📄 売上レポート2024Q3.pdf    │
│                            │    (P.5, チャンク#12)            │
│                            │    [元ドキュメントを開く]        │
│                            │                                  │
│                            │  👍 Good  👎 Bad                 │
...
S3署名付きURLを生成し、ユーザーが元ドキュメントにアクセスできるようにします。
...
export const handler = async (event: APIGatewayProxyEvent) => {
  ...
  // 1. JWT検証・テナントID抽出
  const claims = event.requestContext.authorizer!.claims;
  const tenantId = extractTenantId(claims['cognito:groups']); // "SG-Sales"
  const userId = claims['email'];

  // 2. リクエスト解析
  const { documentKey } = JSON.parse(event.body!); // "SG-Sales/売上レポート2024Q3.pdf"
...
function extractTenantId(groups: string[]): string {
  // "Admin-SG-Sales" または "User-SG-Sales" から "SG-Sales" を抽出
  const group = groups.find(g => g.startsWith('Admin-') || g.startsWith('User-'));
  if (!group) throw new Error('テナントIDが見つかりません');
  return group.replace(/^(Admin-|User-)/, '');
}
```
...
```typescript
// React Component: SourceCitation
...
export const SourceCitation: React.FC<SourceCitationProps> = ({ sources }) => {
  const handleOpenDocument = async (s3Key: string) => {
    try {
      // Lambda経由でS3署名付きURL取得
      const response = await fetch('/api/documents/presigned-url', {
        ...
        body: JSON.stringify({ documentKey: s3Key }),
      });
      ...
      {sources.map((source, index) => (
        <div key={index} className="source-item">
          <div className="source-header">
            📄 {source.fileName} (チャンク#{source.chunkIndex})
          </div>
          <div className="source-preview">
            {source.chunkText.substring(0, 200)}...
          </div>
          <button
            onClick={() => handleOpenDocument(source.s3Key)}
...
```typescript
// React Component: FeedbackButtons
...
export const FeedbackButtons: React.FC<FeedbackButtonsProps> = ({
  ...
  const handleFeedback = async (newRating: 'good' | 'bad') => {
    setRating(newRating);
  ...
  return (
    <div className="feedback-buttons">
      <button
        ...
        className={`btn-feedback ${rating === 'good' ? 'active' : ''}`}
      ...
      <button
        ...
        className={`btn-feedback ${rating === 'bad' ? 'active' : ''}`}
        disabled={rating !== null}
      >
        👎 Bad
      </button>
      <a
        href="https://forms.office.com/r/YOUR_FORM_ID"
        target="_blank"
...
| カテゴリ | 開発要素 | 使用技術 | 工数見積 |
|---------|---------|---------|---------|
| **チャット基本UI** | メッセージ送受信、ストリーミング表示、マークダウンレンダリング | React, TypeScript, SSE | **GenU標準機能**（カスタマイズのみ 3人日） |
| **引用表示機能** | ソース情報表示、チャンクプレビュー、元ドキュメントリンク | React, TypeScript | 5人日 |
| **Lambda（署名付きURL）** | GeneratePresignedUrl、テナント検証、ログ記録 | TypeScript, AWS SDK | 3人日 |
| **フィードバック機能** | Good/Badボタン、DynamoDB保存、外部リンク | React, TypeScript | **GenU標準機能**（外部リンク追加のみ 1人日） |
| **セッション管理** | 会話履歴表示、サイドバーUI | React, TypeScript | **GenU標準機能**（カスタマイズ不要） |
| **インフラ（CDK）** | API Gateway、Lambda、DynamoDB設定 | CDK, TypeScript | 3人日 |

**一般利用者向けチャットUI 小計**: **15人日**

※ GenU標準機能を最大限活用し、カスタム開発を最小限に抑えることでコスト最適化を実現
...
| └ インフラ（CDK）- EventBridge | S3イベント→Lambda トリガー設定 | CDK (TypeScript) | 2人日 |
| **一般利用者向けチャットUI** | | | **15人日** |
| ├ チャット基本UI | メッセージ送受信、ストリーミング表示、マークダウンレンダリング | React, TypeScript, SSE | 3人日 |
| ├ 引用表示機能 | ソース情報表示、チャンクプレビュー、元ドキュメントリンク | React, TypeScript | 5人日 |
| ├ Lambda（GeneratePresignedUrl） | S3署名付きURL生成、テナント検証、ログ記録 | TypeScript, AWS SDK | 3人日 |
| ├ フィードバック機能 | Good/Badボタン、DynamoDB保存、外部リンク | React, TypeScript | 1人日 |
| └ インフラ（CDK） | API Gateway、Lambda、DynamoDB設定 | CDK, TypeScript | 3人日 |
| **システム管理・モニタリング基盤** | | | **10人日** |
| ├ 管理画面UI（追加分） | RAGモニタリング表示部分のみ | React, TypeScript | 3人日 |
| ├ CloudWatch Dashboard | Dashboard定義、Metric Filter、ログ出力 | CDK, TypeScript | 5人日 |
| └ Lambda API（追加分） | モニタリング用API（認証・認可システムとの重複除く） | TypeScript, AWS SDK | 2人日 |
...
**全体工数**: **約163人日**（当社カスタム開発 133人日 + AWS標準機能設定 30人日）

### 工数内訳サマリー

| 大カテゴリ | 小計 | 主要コンポーネント |
|-----------|------|-------------------|
| **認証・認可システム** | 55人日 | マルチテナントリゾルバ、管理者登録フロー、Cognito設定 |
| **LLMゲートウェイ** | 25人日 | 統合API、テナント別フィルタリング、ガバナンスログ |
| **データ前処理パイプライン** | 18人日 | Textract連携、セマンティックチャンキング、Aurora格納 |
| **一般利用者向けチャットUI** | 15人日 | 引用表示、フィードバック機能、S3署名付きURL |
| **システム管理・モニタリング** | 10人日 | CloudWatch Dashboard、RAG関連メトリクス |
| **システム管理者向け管理UI** | 10人日 | 部門管理、ベクトルDB管理（※認証・認可に含む） |
| **AWS標準機能設定** | 30人日 | セキュリティインフラ、モニタリング、暗号化、KB基盤 |
| **合計** | **163人日** | |
...
**作成者**: SI開発チーム | **最終更新**: 2025-12-12 | **バージョン**: 8.0（統合版：一般利用者向けチャットUI・システム管理モニタリング基盤・PH1スコープ明確化）
