...
| 1.2 | グループ割当処理 | API Gateway<br>Cognito Authorizer<br>Lambda<br>Cognito User Pool<br>S3 | ✅ **開発必要** | **バックエンド実装（API + Lambda + インフラ）**<br>- REST API エンドポイント定義<br>- Cognito Authorizer による JWT 検証設定<br>- SystemAdmin グループのみアクセス許可<br>- Lambda: ユーザー存在確認と処理分岐<br>- 既存ユーザー: 直接Cognitoグループに追加<br>- 未ログインユーザー: S3に保留情報を保存<br>- S3バケット作成と30日自動削除ポリシー設定 | インフラ（CDK）<br>API Gateway<br>+<br>Lambda<br>AssignPendingGroup<br>+<br>インフラ（CDK）<br>S3 | CDK<br>TypeScript<br>AWS SDK | 10人日 |
